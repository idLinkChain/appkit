 ...
 */
var rt;
const Ge = window, Qt = Ge.trustedTypes, Fo = Qt ? Qt.emptyScript : "", Yt = Ge.reactiveElementPolyfillSupport, Pt = { toAttribute(t, e) {
  switch (e) {
    case Boolean:
      t = t ? Fo : null;
      break;
...
ue[Wt] = !0, ue.elementProperties = /* @__PURE__ */ new Map(), ue.elementStyles = [], ue.shadowRootOptions = { mode: "open" }, Yt == null || Yt({ ReactiveElement: ue }), ((rt = Ge.reactiveElementVersions) !== null && rt !== void 0 ? rt : Ge.reactiveElementVersions = []).push("1.6.3");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
var nt;
const Xe = window, be = Xe.trustedTypes, Gt = be ? be.createPolicy("lit-html", { createHTML: (t) => t }) : void 0, Nt = "$lit$", Q = `lit$${(Math.random() + "").slice(9)}$`, Ao = "?" + Q, qo = `<${Ao}>`, se = document, Se = () => se.createComment(""), Le = (t) => t === null || typeof t != "object" && typeof t != "function", _o = Array.isArray, Ko = (t) => _o(t) || typeof (t == null ? void 0 : t[Symbol.iterator]) == "function", at = `[ 	
\f\r]`, Ce = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, Xt = /-->/g, Jt = />/g, ee = RegExp(`>|${at}(?:([^\\s"'>=/]+)(${at}*=${at}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), eo = /'/g, to = /"/g, Eo = /^(?:script|style|textarea|title)$/i, ko = (t) => (e, ...o) => ({ _$litType$: t, strings: e, values: o }), l = ko(1), Ye = ko(2), le = Symbol.for("lit-noChange"), T = Symbol.for("lit-nothing"), oo = /* @__PURE__ */ new WeakMap(), ie = se.createTreeWalker(se, 129, null, !1);
function Oo(t, e) {
...
const Qo = (t, e) => {
  ...
  for (let s = 0; s < o; s++) {
    ...
    n += a === Ce ? c + qo : $ >= 0 ? (i.push(d), c.slice(0, $) + Nt + c.slice($) + Q + _) : c + Q + ($ === -2 ? (i.push(void 0), s) : _);
  }
...
class Pe {
  constructor({ strings: e, _$litType$: o }, i) {
    ...
    this.parts = [];
    let n = 0, a = 0;
    const s = e.length - 1, c = this.parts, [d, x] = Qo(e, o);
      ...
      if (r.nodeType === 1) {
        if (r.hasAttributes()) {
          ...
          for (const m of r.getAttributeNames())
            if (m.endsWith(Nt) || m.startsWith(Q)) {
              const _ = x[a++];
              if ($.push(m), _ !== void 0) {
                const f = r.getAttribute(_.toLowerCase() + Nt).split(Q), u = /([.?@])?(.*)/.exec(_);
...
class Ne {
  ...
  g(e) {
    ...
    if (((o = this._$AH) === null || o === void 0 ? void 0 : o._$AD) === n)
      ...
      const a = new Yo(n, this), s = a.u(this.options);
 ...
 */
const L = (t) => t ?? T;
var Qr = Object.defineProperty, ho = Object.getOwnPropertySymbols, Yr = Object.prototype.hasOwnProperty, Gr = Object.prototype.propertyIsEnumerable, mo = (t, e, o) => e in t ? Qr(t, e, { enumerable: !0, configurable: !0, writable: !0, value: o }) : t[e] = o, pt = (t, e) => {
  for (var o in e || (e = {}))
    Yr.call(e, o) && mo(t, o, e[o]);
  if (ho)
    for (var o of ho(e))
...
const Ai = { generate(t, e, o) {
  ...
  c.forEach(({ x: f, y: u }) => {
    ...
    for (let S = 0; S < c.length; S += 1) {
      const N = s * (B - S * 2);
      n.push(Ye`<rect fill="${S % 2 === 0 ? i : r}" height="${N}" rx="${N * M}" ry="${N * M}" width="${N}" x="${O + s * S}" y="${b + s * S}">`);
    }
  });
  ...
  return m.forEach(([f, u]) => {
    ...
    u.forEach((O) => {
      n.push(Ye`<circle cx="${f}" cy="${O}" fill="${i}" r="${s / go}">`);
    });
    ...
    u.forEach(([O, b]) => {
      n.push(Ye`<line x1="${f}" x2="${f}" y1="${O}" y2="${b}" stroke="${i}" stroke-width="${s / (go / 2)}" stroke-linecap="round">`);
    });
  }), n;
...
let F = class extends w {
  ...
  svgTemplate() {
    ...
    return Ye`<svg height="${t}" width="${t}">${Ai.generate(this.uri, t, t / 4)}</svg>`;
  }
  render() {
...
var Ki = Object.defineProperty, Qi = Object.getOwnPropertyDescriptor, me = (t, e, o, i) => {
...
_t.styles = [v.globalCss, Yi], _t = Gi([y("wcm-desktop-wallet-selection")], _t);
